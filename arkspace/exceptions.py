"""
ARKSpace — Custom Exception Hierarchy
...
class ARKSpaceError(Exception):
    """ARKSpace base exception — all app-specific errors inherit from this."""

    def __init__(self, message=None, title=None):
        self.message = message or frappe._("An error occurred in ARKSpace")
        self.title = title or frappe._("ARKSpace Error")
        super().__init__(self.message)


class ValidationError(ARKSpaceError):
...
class NotFoundError(ARKSpaceError):
...
class PermissionError(ARKSpaceError):
...
class ConfigurationError(ARKSpaceError):
...
class IntegrationError(ARKSpaceError):
...
class RateLimitError(ARKSpaceError):
