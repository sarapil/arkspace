...
"""
CAPS Permission Gate for ARKSpace
...
def has_capability(user=None, capability=None):
    """Check if user has a specific ARKSpace capability via CAPS."""
    if not user:
    ...
    if not capability.startswith("ARK_"):
        capability = f"ARK_{capability}"
