...
def seed_data():
    ...
    """
    frappe.logger().info("ARKSpace: Running seed_data()...")

    _seed_settings()
    _seed_roles()

    frappe.db.commit()
    frappe.logger().info("ARKSpace: seed_data() complete.")
...
def _seed_settings():
    ...
    settings_dt = None
    for dt_name in ["AS Settings", "ARKSpace Settings", "Arkspace Settings"]:
...
def _seed_roles():
    ...
    roles = [
        {"role": "AS User", "desk_access": 1},
        {"role": "AS Manager", "desk_access": 1},
        {"role": "AS Admin", "desk_access": 1},
    ]
